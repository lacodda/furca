#!/bin/sh
# furca installer for macOS and Linux:
...
case "$(uname -s)-$(uname -m)" in
    ...
    MINGW*|MSYS*|CYGWIN*)
        ...
        ;;
    *)
        echo "No prebuilt binary for $(uname -s)/$(uname -m); install with: cargo install furca" >&2
        exit 1
        ;;
esac
...
# FURCA_VERSION pins a specific release.
TAG="${FURCA_VERSION:-}"
if [ -z "$TAG" ]; then
...
case "$TAG" in
    v[0-9]*) ;;
    *)
        echo "Cannot resolve the latest release of $REPO - set FURCA_VERSION to a tag like v0.1.0" >&2
        exit 1
        ;;
esac

NAME="furca-$TAG-$TARGET"
...
BIN_DIR="${FURCA_INSTALL_DIR:-$HOME/.local/bin}"
mkdir -p "$BIN_DIR"
install -m 755 "$TMP/$NAME/furca" "$BIN_DIR/furca"
echo "Installed furca $TAG to $BIN_DIR/furca"
